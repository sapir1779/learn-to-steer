...
from pipelines.flux.flux_test_time_optimization import FluxTestTimeOptimization
...
class FluxPipelineDebug(FluxPipeline):
    ...
    def __call__(
        ...
        if relation_classifier is not None:
            test_time_opt = FluxTestTimeOptimization(num_inference_steps, prompt_parser, relation_classifier, 
                                                    self.conf.test_time_optimization, 
                                                    self.attention_cache,
                                                    self.transformer, self.tokenizer_2, 
                                                    self.joint_attention_kwargs,
                                                    guidance, pooled_prompt_embeds, prompt_embeds,
                                                    text_ids, latent_image_ids, prompt, 
                                                    self.conf.workspace_path, device)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if test_time_opt is not None and test_time_opt.should_run(i):
                    latents = test_time_opt.run(latents, timestep, i)
        ...
        if output_type == "latent":
            image = latents
        elif self.conf and self.conf.decode_all_latents:
