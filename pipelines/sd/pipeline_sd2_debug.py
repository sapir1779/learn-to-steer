...
from pipelines.sd.sd2_test_time_optimization import Sd2TestTimeOptimization
...
class Sd2PipelineDebug(StableDiffusionPipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if relation_classifier is not None:
            if self.do_classifier_free_guidance:
                only_text_prompt_embeds = prompt_embeds[1].unsqueeze(dim=0)
            else:
                only_text_prompt_embeds = prompt_embeds
            
            test_time_opt = Sd2TestTimeOptimization(num_inference_steps, prompt_parser, relation_classifier, 
                                                    self.conf.test_time_optimization, 
                                                    self.attention_cache, self.unet, self.tokenizer,
                                                    timestep_cond, self.cross_attention_kwargs,
                                                    added_cond_kwargs, only_text_prompt_embeds,
                                                    prompt, self.conf.workspace_path, device)
            test_time_opt.config.dump(self.conf.workspace_path, "tto_config.json")
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if test_time_opt is not None and test_time_opt.should_run(i):
                    latent_model_input = self.scheduler.scale_model_input(latents, t)
                    latents = test_time_opt.run(latent_model_input, t, i)
        ...
        if test_time_opt is not None:
            test_time_opt.dump_results_data(self.conf.workspace_path, self.conf.sample_prefix)
